-- ========================================================================
-- SCRIPT DE SUPPRESSION DÉFINITIVE - ANCIENS GOURMETS PIZZA YOLO
-- DATE: 2025-01-16
-- RESTAURANT: Pizza Yolo 77 (ID: 1)
-- CATÉGORIE: GOURMETS (ID: 4)
--
-- OBJECTIF: Supprimer définitivement les 5 anciens gourmets individuels
-- PRÉREQUIS: Test bot WhatsApp réussi avec le nouveau produit composite
-- IDs À SUPPRIMER: 367, 368, 369, 370, 371
--
...
BEGIN
    -- Vérifier que les 5 gourmets sont bien inactifs
    ...
    IF v_inactive_count != 5 THEN
        RAISE EXCEPTION 'ERREUR: Tous les gourmets ne sont pas inactifs! Trouvé: % sur 5', v_inactive_count;
    END IF;

    -- Vérifier que le produit composite GOURMETS existe et est actif
    ...
    IF v_composite_exists = 0 THEN
        RAISE EXCEPTION 'ERREUR: Le produit composite GOURMETS n''existe pas ou est inactif!';
    END IF;

    RAISE NOTICE 'Vérifications OK - 5 gourmets inactifs trouvés, produit composite actif';
END $$;
...
-- Supprimer les 5 anciens gourmets
...
BEGIN
    ...
    FROM france_products
    WHERE id IN (367, 368, 369, 370, 371);

    IF v_deleted_count != 0 THEN
        RAISE EXCEPTION 'ERREUR: Suppression incomplète! Il reste % produits', v_deleted_count;
    END IF;

    RAISE NOTICE 'Suppression réussie - 5 gourmets supprimés';
END $$;

-- Vérifier l'état final de la catégorie GOURMETS
SELECT
...
-- ========================================================================
--
-- ✅ 5 anciens gourmets supprimés (367-371)
-- ✅ Leurs options supprimées automatiquement (CASCADE)
-- ✅ Produit composite GOURMETS intact avec 35 options
-- ✅ Modal d'édition affiche uniquement "GOURMETS" (comme OCV)
--
-- ========================================================================
