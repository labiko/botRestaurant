-- ========================================================================
-- SCRIPT DE SUPPRESSION DÉFINITIVE - ANCIENS PANINI PIZZA YOLO
-- DATE: 2025-01-16
-- RESTAURANT: Pizza Yolo 77 (ID: 1)
-- CATÉGORIE: PANINI (ID: 17)
--
-- OBJECTIF: Supprimer définitivement les 5 anciens paninis individuels
-- PRÉREQUIS: Test bot WhatsApp réussi avec le nouveau produit composite
-- IDs À SUPPRIMER: 187, 188, 189, 190, 191
--
...
BEGIN
    -- Vérifier que les 5 paninis sont bien inactifs
    ...
    IF v_inactive_count != 5 THEN
        RAISE EXCEPTION 'ERREUR: Tous les paninis ne sont pas inactifs! Trouvé: % sur 5', v_inactive_count;
    END IF;

    -- Vérifier que le produit composite PANINI existe et est actif
    ...
    IF v_composite_exists = 0 THEN
        RAISE EXCEPTION 'ERREUR: Le produit composite PANINI n''existe pas ou est inactif!';
    END IF;

    RAISE NOTICE 'Vérifications OK - 5 paninis inactifs trouvés, produit composite actif';
END $$;
...
-- Supprimer les 5 anciens paninis
...
BEGIN
    ...
    FROM france_products
    WHERE id IN (187, 188, 189, 190, 191);

    IF v_deleted_count != 0 THEN
        RAISE EXCEPTION 'ERREUR: Suppression incomplète! Il reste % produits', v_deleted_count;
    END IF;

    RAISE NOTICE 'Suppression réussie - 5 paninis supprimés';
END $$;

-- Vérifier l'état final de la catégorie PANINI
SELECT
...
-- ========================================================================
--
-- ✅ 5 anciens paninis supprimés (187-191)
-- ✅ Leurs options supprimées automatiquement (CASCADE)
-- ✅ Produit composite PANINI intact avec 35 options
-- ✅ Modal d'édition affiche uniquement "PANINI" (comme OCV)
--
-- ========================================================================
