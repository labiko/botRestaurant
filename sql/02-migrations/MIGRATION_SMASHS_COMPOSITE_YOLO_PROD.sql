-- ========================================================================
-- SCRIPT DE MIGRATION - SMASHS VERS ARCHITECTURE COMPOSITE
-- DATE: 2025-01-16
-- RESTAURANT: Pizza Yolo 77 (ID: 1)
-- CATÉGORIE: SMASHS (ID: 5)
--
-- OBJECTIF: Migrer de 6 produits individuels vers 1 produit composite
-- MODÈLE: Architecture OCV (1 produit avec options en groupes)
-- IDs À MIGRER: 218, 219, 220, 221, 222, 223
--
-- ARCHITECTURE CIBLE:
...
BEGIN
    -- Vérifier que la catégorie SMASHS existe pour Pizza Yolo
    ...
    IF v_category_exists = 0 THEN
        RAISE EXCEPTION 'ERREUR: Catégorie SMASHS (ID: 5) n''existe pas pour Pizza Yolo (ID: 1)';
    END IF;

    -- Vérifier que les 6 produits existent
    ...
    IF v_products_count != 6 THEN
        RAISE EXCEPTION 'ERREUR: Nombre de produits SMASHS incorrect! Trouvé: % sur 6', v_products_count;
    END IF;

    RAISE NOTICE 'Vérifications OK - Catégorie et 6 produits SMASHS trouvés';
END $$;

-- ========================================================================
-- ÉTAPE 1: CRÉATION DU PRODUIT COMPOSITE "SMASHS"
-- ========================================================================
...
VALUES (
    1,                    -- Pizza Yolo
    5,                    -- SMASHS
    'SMASHS',             -- Nom du produit composite
...
BEGIN
    ...
    RAISE NOTICE 'Produit composite SMASHS créé avec ID: %', v_composite_id;
...
-- ========================================================================
-- ÉTAPE 2: CRÉATION DU GROUPE "PLATS" (6 SMASHS)
-- ========================================================================

-- Insérer les 6 smashs comme options dans le groupe "Plats"
DO $$
DECLARE
...
BEGIN
    ...
    VALUES (v_product_id, 'Plats', 'SMASH CLASSIC', '🍔', 8.90, 1, true,
            'Pain buns, steak smash beef 70g, cheddar, crudités, sauce au choix');

    -- 2. L'ORIGINAL - 11.90€
    INSERT INTO france_product_options (product_id, option_group, option_name, icon, price_modifier, display_order, is_active, composition)
    VALUES (v_product_id, 'Plats', 'L''ORIGINAL', '🍔', 11.90, 2, true,
            'Pain, 2 steaks smash beef 70g, double cheddar, crudités, sauce au choix');

    -- 3. SMASH SIGNATURE - 12.90€
    INSERT INTO france_product_options (product_id, option_group, option_name, icon, price_modifier, display_order, is_active, composition)
    VALUES (v_product_id, 'Plats', 'SMASH SIGNATURE', '✨', 12.90, 3, true,
            'Pain, 2 steaks smash beef 70g, double cheddar, oignons confits, sauce au choix');

    -- 4. SMASH BACON - 11.90€
    INSERT INTO france_product_options (product_id, option_group, option_name, icon, price_modifier, display_order, is_active, composition)
    VALUES (v_product_id, 'Plats', 'SMASH BACON', '🥓', 11.90, 4, true,
            'Pain buns, steak smash beef 70g, cheddar, bacon, œuf, crudités, oignons caramélisés');

    -- 5. LE SMASH MIELLEUX - 11.90€
    INSERT INTO france_product_options (product_id, option_group, option_name, icon, price_modifier, display_order, is_active, composition)
    VALUES (v_product_id, 'Plats', 'LE SMASH MIELLEUX', '🍯', 11.90, 5, true,
            'Pain buns, steak smash beef 70g, chèvre, miel, cheddar, crudités, noix');

    -- 6. CHICKEN CRAZY - 11.90€
    INSERT INTO france_product_options (product_id, option_group, option_name, icon, price_modifier, display_order, is_active, composition)
    VALUES (v_product_id, 'Plats', 'CHICKEN CRAZY', '🐔', 11.90, 6, true,
            'Pain buns, filet de poulet pané, cheddar, oignons confits, alloco, coleslaw, sauce samouraï');

    RAISE NOTICE '6 smashs ajoutés au groupe Plats';
END $$;
...
-- Désactiver les 6 anciens produits individuels (NE PAS SUPPRIMER)
...
-- Vérifier l'état de la catégorie SMASHS
SELECT
...
-- ========================================================================
--
-- ✅ 1 produit composite "SMASHS" créé
-- ✅ 6 options dans groupe "Plats" (avec compositions)
-- ✅ 12 options dans groupe "Boisson 33CL incluse"
-- ✅ 16 options dans groupe "Sauces"
-- ✅ 2 options dans groupe "Suppléments"
-- ✅ TOTAL: 36 options
-- ✅ Workflow: universal_workflow_v2 (4 steps)
-- ✅ 6 anciens produits désactivés (218-223)
--
-- ========================================================================
