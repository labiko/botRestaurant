/**
 * 🇫🇷 Bot WhatsApp France - Version multi-restaurants
 * Optimisé pour les restaurants en France (Pizza Yolo 77, etc.)
 */
...
class SimpleSession {
  ...
  static async create(phoneNumber: string, chatId: string, restaurantId?: number) {
    const { data } = await supabase
      .from('france_user_sessions')
      .insert({
        phone_number: phoneNumber,
        chat_id: chatId,
        restaurant_id: restaurantId || 1, // Pizza Yolo par défaut
  ...
  static async update(sessionId: number, updates: any) {
    const { data } = await supabase
      .from('france_user_sessions')
      .update({
        ...updates,
...
// Gestion des restaurants France
class FranceRestaurant {
  static async getActiveRestaurants() {
    console.log('🔍 Requête restaurants France actifs...');
    ...
    console.log('📊 Résultats:', { 
      ...
      error: error?.message,
      restaurants: restaurants?.map(r => ({ name: r.name, city: r.city })) 
    });
...
// Fonction de formatage prix pour la France
