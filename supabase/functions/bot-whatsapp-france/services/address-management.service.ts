...
export class AddressManagementService {
  ...
  async buildAddressSelectionMessage(phoneNumber: string): Promise<AddressSelectionMessage> {
    try {
      ...
      let message = '📍 **Livraison :**\n\n';
      
      addresses.forEach((address, index) => {
        const number = index + 1;
        const emoji = address.is_default ? '⭐' : this.getAddressEmoji(address.address_label);
        const label = address.address_label;
        // Raccourcir l'adresse pour mobile (plus court pour format compact)
        let shortAddress = address.full_address.replace(/, France$/, '');
        if (shortAddress.length > 35) {
          shortAddress = shortAddress.substring(0, 32) + '...';
        }
        
        message += `**${number}** ${emoji} ${label} (${shortAddress})\n`;
      });
      
      const nextNumber = addresses.length + 1;
      message += `**${nextNumber}** ➕ Nouvelle adresse\n\n`;
      message += `*Tapez votre choix (1-${nextNumber})*`;
