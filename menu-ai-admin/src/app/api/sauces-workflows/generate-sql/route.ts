...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Récupérer les sauces de la catégorie SAUCES
    const { data: saucesCategory, error: categoryError } = await supabase
    ...
    if (categoryError || !saucesCategory) {
      return NextResponse.json(
        { success: false, error: 'Catégorie SAUCES non trouvée' },
        { status: 404 }
      );
    }

    // 2. Récupérer toutes les sauces
    const { data: sauces, error: saucesError } = await supabase
      .from('france_products')
      .select('id, name, price_on_site_base, icon')
      .eq('restaurant_id', restaurant_id)
      .eq('category_id', saucesCategory.id)
      .order('display_order');

    if (saucesError) {
      return NextResponse.json(
        { success: false, error: saucesError.message },
        { status: 500 }
      );
    }
    ...
    if (!sauces || sauces.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Aucune sauce trouvée' },
        { status: 404 }
      );
    }
    ...
    const { data: products, error: productsError } = await supabase
      ...
      `)
      .in('id', product_ids)
      .eq('options.option_group', 'Sauces');
    ...
    for (const product of (products || [])) {
      ...
      verificationSQL += `  AND po.option_group = 'Sauces'\n`;
      verificationSQL += `ORDER BY po.display_order;\n\n`;
    }

    // 5. Générer le script d'exécution
    let executionSQL = `-- ========================================\n`;
    executionSQL += `-- SCRIPT D'EXÉCUTION - REMPLACEMENT SAUCES\n`;
    ...
    executionSQL += `-- Sauces: ${sauces.length}\n`;
    executionSQL += `-- ⚠️ ATTENTION: Ce script va supprimer et remplacer les sauces\n`;
    executionSQL += `-- ========================================\n\n`;
    executionSQL += `BEGIN;\n\n`;
    ...
    for (const product of (products || [])) {
      ...
      // DELETE
      executionSQL += `-- Supprimer les anciennes sauces\n`;
      executionSQL += `DELETE FROM france_product_options\n`;
      executionSQL += `WHERE product_id = ${product.id}\n`;
      executionSQL += `  AND option_group = 'Sauces';\n\n`;

      // INSERT
      executionSQL += `-- Insérer les nouvelles sauces\n`;
      sauces.forEach((sauce, index) => {
        ...
        executionSQL += `  'Sauces',\n`;
        executionSQL += `  '${sauce.name.replace(/'/g, "''")}',\n`;
        executionSQL += `  0,\n`; // Prix = 0€ (inclus dans le menu)
        executionSQL += `  ${index + 1},\n`;
        executionSQL += `  ${groupOrder},\n`;
        executionSQL += `  true,\n`;
        executionSQL += `  true,\n`;
        executionSQL += `  ${sauce.icon ? `'${sauce.icon}'` : 'NULL'}\n`;
        executionSQL += `);\n\n`;
      });
    }
    ...
    executionSQL += `  COUNT(po.id) as nb_sauces\n`;
    executionSQL += `FROM france_products p\n`;
    executionSQL += `LEFT JOIN france_product_options po ON po.product_id = p.id AND po.option_group = 'Sauces'\n`;
