...
export async function GET(request: NextRequest) {
  try {
    ...
    // Récupérer tous les produits avec step "Boissons" dans leur configuration
    const { data: products, error } = await supabase.rpc('get_products_with_boissons_step', {
    ...
    if (error) {
      ...
      // Filtrer les produits qui ont un step avec option_group "Boissons"
      const filteredProducts = productsData
        ?.filter((product: any) => {
          try {
            ...
            return steps.some((step: any) =>
              step.option_groups && step.option_groups.includes('Boissons')
            );
          } catch (e) {
            return false;
          }
        ...
        .map((product: any) => ({
          ...
          boissons_count: product.options?.filter((o: any) => o.option_group === 'Boissons').length || 0
        })) || [];
